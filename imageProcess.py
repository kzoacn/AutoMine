...
def get_frame():
    ...
    if hwnd:
        ...
        blocks_img = [[0 for i in range(blocks_y)] for j in range(blocks_x)]

        for y in range(blocks_y):
            for x in range(blocks_x):
                blocks_img[x][y] = crop_block(img, x, y)
                croped = pil_to_cv(blocks_img[x][y])
                #cv2.imshow("Sweeper Screenshot", croped)
                #cv2.waitKey(10)
        
        croped = pil_to_cv(blocks_img[0][0])
        cv2.imshow('Hi ',croped)
        cv2.waitKey(100)
